...
def graph_list(_df_list, x_list, y_list, group_list, facet,
    ...
    for index, (_df, x, y, group, max_over, pick) in\
        ...
        if pick:
            ...
            for _fvalues, _df in tmp.groupby(facet):
                ...
                # if index == 0:
                if len(axs_index) > 0:
                    ax = plt.subplot(2, 2, plt_nr, sharey=axs_index[0])
                else:
                    ax = plt.subplot(2, 2, plt_nr)
                axs_index.append(ax)
                axs.append(ax)
                # else:
                #     ax = axs[plt_nr - 1]
    ...
    if not per_ax_legend:
        ...
        fig.legend(flip(handle_list, 3), flip(label_list, 3),
