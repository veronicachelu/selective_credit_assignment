...
def add_subplot(data, num_ticks, exclude,
    ...
    for agent in agents:
        ...
        for hyper_values in itertools.product(*agents_params.values()):
            for v, k in zip(hyper_values, agents_params.keys()):
                hyper[k] = v
            #   print(hyper_values)
            if hyper["option_alpha_v"] in exclude:
                i += 1
                continue
            if len(data[i]) == 0:
                continue
            ...
            agent_label = (r'{}{}{}{}'.format("X-" if hyper["option_eta_f"] == 0 else "",
                           ...
                           "{}{}{}{}{}{}".format(
                               ...
                               # r', use $\rho$' if hyper["use_post_corr"] else r'',
                               # r',$\alpha_q={}$'.format(hyper["alpha_q"]) if hyper["alpha_q"] is not None and ("alpha_v" in list(agents_params.keys()) and len(agents_params["alpha_v"]) > 1 or "alpha_v" not in list(agents_params.keys())) else '',
                               r',action_repeats={}'.format(hyper["action_repeats"]) if hyper[
                                                                                            "action_repeats"] is not None and len(
                                   agents_params["action_repeats"]) > 1 else '',
                               r',$\eta^o_z={}$'.format(hyper["option_eta_z"]) if hyper[
                                                                                      "option_eta_z"] is not None and (
                                                                                              "option_eta_z" in list(
                                                                                          agents_params.keys()) and len(
                                                                                          agents_params[
                                                                                              "option_eta_z"]) > 1 or "option_eta_z" not in list(
                           ...
                           # r',$\beta^o={}$'.format(hyper["option_beta"]) if hyper["option_beta"] < 100 and len(agents_params["option_beta"]) > 1 else ''))
                           )
            linestyle = ':' if hyper['option_eta_v'] == 1 else '-'
            color = i_q % len(COLORS) if hyper['option_eta_v'] == 1 else i_qet % len(COLORS)
            ax.plot(x_mean, mean,
                    ls=linestyle, lw=5,
            ...
            i += 1
            i_q += 1 if hyper['option_eta_v'] == 1 else 0
            i_qet += 1 if hyper['option_eta_v'] == 0 else 0
