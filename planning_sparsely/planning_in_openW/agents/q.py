...
def update_option(obs, s, next_s, x, w_x, next_obs, reward, next_x,
    ...
    option_dw_v, option_v_opt_state = option_v_scale_fn(option_dw_v,
                                                        state=option_state.v_opt_state)
    option_dw_z, option_z_opt_state = option_z_scale_fn(option_dw_z,
                                                        state=option_state.z_opt_state)
    ...
    # Compute trace decay to apply at the next time step
    option_trace_decay = discount * trace_parameter #* option_rho_post
...
def init(
    ...
    def policy(rng_key, obs, s, agent_state):
        ...
        next_a = jnp.where(agent_state.counter % action_repeats == 0,
                           next_a, prev_a)
        next_a_prob = pi[next_a] #jnp.where(agent_state.counter % action_repeats == 0,
