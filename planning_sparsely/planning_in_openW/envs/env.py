...
class GridWorld(base.Environment):
    ...
    def _fill_reward_matrix(self):
        ...
        for next_s in range(self._num_states):
            ...
            if next_x == self._goal_G_x and next_y == self._goal_G_y:
                self._reward_matrix[next_s] = self._rval_G
            if next_x == self._goal_g_x and next_y == self._goal_g_y:
                self._reward_matrix[next_s] = self._rval_g
    ...
    def _reset(self) -> dm_env.TimeStep:
        self._curr_idx = self._nrng.choice(self._start_states,
    ...
    def is_terminal(self, _curr_x, _curr_y):
        ...
        """
        if (_curr_x == self._goal_G_x and _curr_y == self._goal_G_y) or \
                (_curr_x == self._goal_g_x and _curr_y == self._goal_g_y):
            return True
        else:
            return False
    ...
    def _option_q_backup_sparse(self, q_values, option_interest, v_star, discount=0.99):
        dS = self._num_states
        dA = self._num_actions
        #   discounts = discount * self._discount_matrix
        new_q_values = np.zeros_like(q_values)
            ...
            for a in range(dA):
                ...
                for ns, prob in enumerate(self._transition_matrix[s][a]):
                    ...
                    if self._use_q_for_reward:
                        option_reward *= v_star[ns]
                    option_discount = option_interest[ns] * discount
                    new_q_value += prob * (option_reward * self._reps +
    ...
    def options_q_iteration(self, option_interests=None, num_itrs=100, v_star=None, **kwargs):
    ...
    def get_v_pi(self, pi, discount=None, option_epsilon=0):
        dS = self._num_states
        for s in range(dS):
            pi[s] = (1 - option_epsilon) * pi[s] + option_epsilon / pi[s].shape[-1]
        discount = 1. if discount is None else discount
        discounts = discount * self._discount_matrix
        t_matrix = self._transition_matrix
        ...
        v = self._discount_matrix * np.linalg.inv(np.eye(self._num_states) - disc_ppi).dot(rpi)
        return v
