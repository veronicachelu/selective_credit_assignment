...
def plot_sa_values_4(env, policy, option_interest,
                     q_values1, q_values2, q_values3, text_values=True,
                     invert_y=True,
                     update=False,
                     title=None,
                     fontsize=25,
    ...
    for i, q_values in enumerate([q_values3, q_values1, q_values2]):
...
def plot_sa_values_pio(env, q_pi, options_q_pi,
    ...
    ax = fig.gca()  # plt.subplot(1, 1, 1)

    for oo in range(len(option_interests)):
        q_values = options_q_pi[oo]
        normalized_values = q_values
        normalized_values = normalized_values - np.min(normalized_values) + 0.9
        normalized_values = normalized_values / np.max(normalized_values)
